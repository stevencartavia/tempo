...
impl NetworkTransactionBuilder<TempoNetwork> for TempoTransactionRequest {
    fn complete_type(&self, ty: TempoTxType) -> Result<(), Vec<&'static str>> {
        match ty {
            ...
            | TempoTxType::Eip1559
            | TempoTxType::Eip7702 => {
                NetworkTransactionBuilder::<alloy_network::Ethereum>::complete_type(
                    &self.inner,
    ...
    fn can_submit(&self) -> bool {
        NetworkTransactionBuilder::<alloy_network::Ethereum>::can_submit(&self.inner)
    }

    fn can_build(&self) -> bool {
        NetworkTransactionBuilder::<alloy_network::Ethereum>::can_build(&self.inner)
            || self.can_build_aa()
    }
    ...
    fn output_tx_type(&self) -> TempoTxType {
            ...
            || self.fee_payer_signature.is_some()
        {
            ...
            match NetworkTransactionBuilder::<alloy_network::Ethereum>::output_tx_type(&self.inner)
            {
    ...
    fn output_tx_type_checked(&self) -> Option<TempoTxType> {
        match self.output_tx_type() {
            ...
            | TempoTxType::Eip1559
            | TempoTxType::Eip7702 => {
                NetworkTransactionBuilder::<alloy_network::Ethereum>::output_tx_type_checked(
                    &self.inner,
                )?
                .try_into()
                .ok()
